...
def depth_estim_v1(inputs,
  ...
  with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
    ...
    net = tf.reshape(net, [-1, 2560])
    end_point = 'fully_connected'
    net=slim.fully_connected(net, 4096, tf.nn.relu)
    end_points[end_point] = net
    end_point = 'fully_connected_1'
    # output height 55 width 74
    net=slim.fully_connected(net, 55*74, tf.nn.relu)
    end_points[end_point] = net
    with tf.variable_scope('FC'): 
      end_point = 'control_1'
      net = slim.fully_connected(net, 100, scope=end_point)
      end_points[end_point] = net
      # net = slim.dropout(net, keep_prob=dropout_keep_prob, scope='Dropout_1b')
      end_point = 'control_2'
      logits = slim.fully_connected(net, num_classes, activation_fn=tf.tanh, scope=end_point)
...
def arg_scope(weight_decay=0.00004, stddev=0.1):
  ...
  with slim.arg_scope([slim.conv2d, slim.fully_connected],
    ...
    with slim.arg_scope(
        ...
        biases_initializer=tf.truncated_normal_initializer(stddev=stddev)) as sc:
